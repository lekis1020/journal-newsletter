...
function sendSummariesToEmail(spreadsheet) {
  try {
    ...
    // 이메일 제목
    const emailSubject = `[Ajou Allergy Journal Letter] ${searchPeriod} - 총 ${validCount}개 논문`;

    // 이메일 본문 시작
    let emailBody = `<div style="font-family: Arial, sans-serif;">`;
    emailBody += `<h4>최근 ${CONFIG.DAYS_RANGE}일 간 (${searchPeriod}) Allergy and Immunology 관련 Meta-Analysis, RCT, Review, Systematic Review 요약 </h4>`;
    ...
    // 이메일 본문 마무리
    emailBody += `<hr style="margin: 20px 0;">`;
    emailBody += `<p> <논문 검색 조건> </p>`;
    emailBody += '<p> "asthma", "urticaria", "angioedema", "atopic dermatitis","drug allergy","anaphylaxis" 를 논문 이름에 가지고 있는 논문들을 다음 저널에서 검색합니다. <br> "The Journal of allergy and clinical immunology","Allergy", "Annals of allergy, asthma & immunology", "Clinical and experimental allergy", "The journal of allergy and clinical immunology. In practice", "Allergology international","The World Allergy Organization journal", "NEJM evidence", "The New England journal of medicine", "The Lancet. Respiratory medicine", "American journal of respiratory and critical care medicine", "Journal of Investigational Allergology and Clinical Immunology", "Frontiers in Immunology", "Allergy and asthma proceedings", "Allergy, asthma, and clinical immunology", "Allergy, Asthma and Immunology Research", "Allergy, Asthma and Respiratory Disease" </p>';
