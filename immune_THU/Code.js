...
const CONFIG = {
  ...
  JOURNALS: [
    ...
    "Clinical and experimental allergy","Clin Exp Allergy", "Clinical and Translational Immunology",
    ...
    "Allergy, Asthma and Immunology Research","Allergy Asthma Immunol Res",
...
function fetchPubMedWeeklyAndSave() {
  ...
  const meshTerms = ["immunologic deficiency","immunodeficiency","CVID","x-linked agammaglobulinemia", "agammaglobulinemia","agmmaglobulinemia, X-linked","immunoglobulin A deficiency","immunologic deficiency syndromes","selective IgA deficiency","hyper-IgM syndrome","hyper-IgM immunodeficiency syndrome"];
  const meshQuery = meshTerms.map(term => `"${term}"[MeSH Terms]`).join(" OR ");
  const diseaseTerms = ["immunologic deficiency", "Primary immunodeficiency diseases","immune dysregulation","common variable immunodeficiency", "severe combined immunodeficiency", "selective IgA deficiency", "X-linked agammaglobulinemia","eosinophilia","CVID","inborn errors of immunity","B cell","T cell","vaccine"];
  ...
  const finalQuery = `(((${journalQuery}) AND ((${meshQuery}) OR (${diseaseQuery})) AND (${dateRange})) `;
...
function saveResultsToSheet(data) {
  try {
    ...
    const fileName = `immune_THU_${formattedDate}`;
