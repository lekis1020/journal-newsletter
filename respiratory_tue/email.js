...
function formatSummaryForEmail(summary) {
  ...
  return lines.map(line => {
    ...
    if (isTagLine) {
      baseStyle.push("color:#0369A1", "font-weight:600");
    }
...
function sendSummariesToEmail(spreadsheet) {
  try {
    ...
    // ✅ 이메일 제목: 필터링된 논문 수 사용
    const subject = `[Ajou Allergy Journal Letter] Respiratory disease ${searchPeriod} - 총 ${filteredData.length}개 논문`;
    ...
    <h4 style="
      ...
      color: #0C4A6E;
    ">
      최근 ${CONFIG.DAYS_RANGE}일 간 (${searchPeriod}) Asthma/Rhinitis/Sinusitis 논문 요약</h4>`;
    emailBody += `
    <p style="
      font-size: 16px;
      font-weight: 600;
    ...
    for (let i = 0; i < filteredData.length; i++) {
      ...
      // 논문 카드
      emailBody += `<div style="margin-bottom: 30px; border: 1px solid #BFDBFE; padding: 15px; border-radius: 5px; background-color: #FFFFFF;">`;
      // 제목 헤더
      emailBody += `<div style="border-bottom: 2px solid #0369A1; padding-bottom: 10px; margin-bottom: 10px;">`;
      emailBody += `<div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; font-size: 18px; font-weight: bold; color: #0C4A6E; margin-bottom: 10px;">📔: ${title}</div>`;
      emailBody += `</div>`;

      // 요약 내용 (이모지 구조화 렌더링)
      emailBody += `<div style="font-size: 16px; line-height: 1.7; color: #0C4A6E; background-color: #F0F9FF; padding: 12px 14px; border-left: 4px solid #0369A1;">`;
      emailBody += formatSummaryForEmail(summary);
      emailBody += `</div>`;

      // 링크
      emailBody += `<div style="margin-top: 10px; font-size: 14px; color: #0C4A6E;">`;
      const myAllergyLink = buildMyAllergyLink(pmid, journal);
      emailBody += `<strong>링크:</strong> <a href="${pubmedLink}" target="_blank" style="color: #0369A1;">PubMed</a>`;
      if (myAllergyLink) {
        emailBody += ` &nbsp;|&nbsp; <a href="${myAllergyLink}" target="_blank" style="color: #0369A1;">My Allergy에서 보기</a>`;
      }
      emailBody += `</div>`;
      emailBody += `</div>`;
    }
