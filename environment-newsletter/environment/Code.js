...
const CONFIG = {
  OPENAI_API_KEY: '', // Script Properties의 OPENAI_API_KEY 사용 권장
  PUBMED_API_KEY: '', // Script Properties의 PUBMED_API_KEY 사용 권장
  ...
  ENVIRONMENT_TITLE_ABSTRACT_TERMS: [
    "climate change",
    "air pollution",
    "wildfire",
    "particulate matter",
  ...
  ALLERGY_RESP_TITLE_ABSTRACT_TERMS: [
    "asthma",
    "allergic",
    "atopic",
    "rhinitis",
  ...
  EMAIL_TO: '',  // Script Properties에서 가져옴
...
function getConfigSecret(key, fallback) {
  const value = PropertiesService.getScriptProperties().getProperty(key);
  return value || fallback || '';
}

function getRequiredSecret(key, fallback) {
  const value = getConfigSecret(key, fallback);
  if (!value) {
    throw new Error(`Script Properties에 ${key} 값이 필요합니다.`);
  }
  return value;
}

function getSearchPeriodLabel(baseDate = new Date()) {
  const startDate = new Date(baseDate);
  startDate.setDate(baseDate.getDate() - CONFIG.DAYS_RANGE);
  const formatKoreanDate = date => `${date.getMonth() + 1}월 ${date.getDate()}일`;
  return `${formatKoreanDate(startDate)}부터 ${formatKoreanDate(baseDate)}까지`;
}

function buildNewsletterSubject() {
  return `${CONFIG.EMAIL_SUBJECT_PREFIX} ${CONFIG.TOPIC_LABEL} ${getSearchPeriodLabel()} `;
}
...
function setRuntimeSecrets(openaiApiKey, pubmedApiKey, emailRecipients, emailTo) {
  const scriptProps = PropertiesService.getScriptProperties();
  const updates = {};
  if (openaiApiKey) updates.OPENAI_API_KEY = openaiApiKey;
  if (pubmedApiKey) updates.PUBMED_API_KEY = pubmedApiKey;
  if (emailRecipients) updates.EMAIL_RECIPIENTS = emailRecipients;
  if (emailTo) updates.EMAIL_TO = emailTo;

  if (Object.keys(updates).length === 0) {
    return '업데이트할 Script Properties 값이 없습니다.';
  }

  scriptProps.setProperties(updates, false);
  return `Script Properties 업데이트 완료: ${Object.keys(updates).join(', ')}`;
}

function getRuntimeSecretsStatus() {
  const scriptProps = PropertiesService.getScriptProperties();
  const keys = ['OPENAI_API_KEY', 'PUBMED_API_KEY', 'EMAIL_RECIPIENTS', 'EMAIL_TO'];
  return keys.reduce((acc, key) => {
    acc[key] = Boolean(scriptProps.getProperty(key));
    return acc;
  }, {});
}

function pingExecutionApi() {
  return 'ok';
}
...
function buildFieldQuery(terms, field) {
...
function buildPubTypeQuery() {
  return buildFieldQuery(CONFIG.PUB_TYPES, 'Publication Type');
}
...
function buildDateRangeQuery() {
  ...
  return `"${formatDateForPubMed(oneWeekAgo)}"[PDAT] : "${formatDateForPubMed(today)}"[PDAT]`;
}

function buildPubMedSearchQueries() {
  const tierABJournals = [...CONFIG.JOURNAL_TIERS.A, ...CONFIG.JOURNAL_TIERS.B];
  const tierCJournals = CONFIG.JOURNAL_TIERS.C;
  const pubTypeQuery = buildPubTypeQuery();
  const dateRange = buildDateRangeQuery();

  const envTitleAbstractQuery = buildFieldQuery(CONFIG.ENVIRONMENT_TITLE_ABSTRACT_TERMS, 'Title/Abstract');
  const allergyTitleAbstractQuery = buildFieldQuery(CONFIG.ALLERGY_RESP_TITLE_ABSTRACT_TERMS, 'Title/Abstract');
  const envMeshQuery = buildFieldQuery(CONFIG.ENVIRONMENT_MESH_TERMS, 'MeSH Terms');
  const allergyMeshQuery = buildFieldQuery(CONFIG.ALLERGY_RESP_MESH_TERMS, 'MeSH Terms');

  const passAQuery = `(${buildJournalQuery(tierABJournals)}) AND (${pubTypeQuery}) AND ((${envTitleAbstractQuery}) OR (${envMeshQuery})) AND (${dateRange})`;
  const passBQuery = `(${buildJournalQuery(tierCJournals)}) AND (${pubTypeQuery}) AND ((${allergyTitleAbstractQuery}) OR (${allergyMeshQuery})) AND (${dateRange})`;

  return [passAQuery, passBQuery];
}

function getAllTargetJournals() {
  return [...CONFIG.JOURNAL_TIERS.A, ...CONFIG.JOURNAL_TIERS.B, ...CONFIG.JOURNAL_TIERS.C];
}

function getJournalListText() {
  return getAllTargetJournals().join(', ');
}

function getEnvironmentMeshTermText() {
  return CONFIG.ENVIRONMENT_MESH_TERMS.join(', ');
}

function searchPubMedIds(query, retmax, pubmedApiKey) {
  const encodedQuery = encodeURIComponent(query);
  const apiKeyParam = pubmedApiKey ? `&api_key=${pubmedApiKey}` : '';
  const url = `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=${encodedQuery}&retmax=${retmax}&sort=pubdate&retmode=json${apiKeyParam}`;
  let attempt = 0;
  ...
  while (attempt < CONFIG.MAX_RETRIES) {
    try {
      ...
      console.log(`PubMed 검색 재시도 ${attempt}/${CONFIG.MAX_RETRIES}, 대기: ${delay}ms`);
    }
  }
...
function fetchAndSummarizeAll() {
  const spreadsheet = fetchPubMedWeeklyAndSave();  // 스프레드시트 객체 반환
  if (spreadsheet) {
...
function fetchPubMedWeeklyAndSave() {
  // 1. 검색 쿼리 준비 (2-pass 검색)
  const queries = buildPubMedSearchQueries();
  const pubmedApiKey = getConfigSecret('PUBMED_API_KEY', CONFIG.PUBMED_API_KEY);
  const perQueryMax = Math.max(50, Math.ceil(CONFIG.MAX_RESULTS / queries.length) + 20);
  const uniqueIds = new Set();

  try {
    queries.forEach((query, idx) => {
      const idList = searchPubMedIds(query, perQueryMax, pubmedApiKey);
      console.log(`[검색 패스 ${idx + 1}] 결과 ${idList.length}건`);
      console.log(`[검색 패스 ${idx + 1}] query: ${query}`);
      idList.forEach(id => uniqueIds.add(id));
    });

    const mergedIds = Array.from(uniqueIds).slice(0, CONFIG.MAX_RESULTS);
    ...
    console.log(`중복 제거 후 총 ${mergedIds.length}개의 논문 ID를 가져왔습니다.`);
    const results = fetchPubMedData(mergedIds);
...
function saveResultsToSheet(data) {
  try {
    ...
    const fileName = `environment_fri_${formattedDate}`;
...
function callGPT(prompt) {
  const openAiApiKey = getRequiredSecret('OPENAI_API_KEY', CONFIG.OPENAI_API_KEY);
...
• 🎯: <b>IgE</b>는 알레르기성 기도 질환에서 핵심 분자로, T2 inflammation 및 기도 상피 내 리모델링 과정에서 중요한 역할을 합니다. IgE와 기도 상피 간 복잡한 상호작용 네트워크에 대한 더 깊은 이해는 천식 병리생리학에 대한 이해를 향상시킬 것입니다.
...
• 연구 방법: 분자 endotype, 임상적 phenotype, 피부 microbiome, 진단 도구 및 치료법 발전에 관한 최신 연구 문헌을 종합적으로 검토한 리뷰 논문
