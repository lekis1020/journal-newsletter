...
function fetchSummarizeAndSendByEmail() {
  try {
    ...
    const spreadsheet = fetchPubMedWeeklyAndSave_POST();
...
function sendNoResultsEmail(message) {
  ...
  const emailSubject = `[Ajou Allergy Journal Letter] Cutaneous diseases ${searchPeriod} `;

  // 이메일 본문 시작
  let emailBody = `<div style="font-family: Arial, sans-serif;">`;
  emailBody += `<h4>최근 ${CONFIG.DAYS_RANGE}일 간 (${searchPeriod}) Allergy and Immunology - Cutaneous diseases 논문 요약 </h4>`;
