...
function sendSummariesToEmail(spreadsheet) {
  try {
    ...
    for (let i = 0; i < filteredData.length; i++) {
      ...
      // 링크 및 구분선 (PubMed + My Allergy 포털 딥링크)
      const myAllergyLink = buildMyAllergyLink(pmid, journal);
      emailBody += `<div style="margin-top: 10px; font-size: 14px; color: #134E4A;">`;
      emailBody += `<strong>링크:</strong> <a href="${pubmedLink}" target="_blank" style="color: #0891B2;">PubMed</a>`;
      if (myAllergyLink) {
        emailBody += ` &nbsp;|&nbsp; <a href="${myAllergyLink}" target="_blank" style="color: #0891B2;">My Allergy에서 보기</a>`;
      }
      emailBody += `</div>`;
      emailBody += `</div>`;
    }
