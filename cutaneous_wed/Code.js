...
/**
 * 수요일 cutaneous 
 * - Urticaria, angioedema, atopic dermatitis, anaphylaxis 
...
const CONFIG = {
  ...
  JOURNALS: [
    ...
    "Allergy, asthma, and clinical immunology","Current Opinion in Allergy and Clinical Immunology",
    "Journal of immunotoxicology","International journal of dermatology","Contact dermatitis",
...
function fetchAndSummarizeAll() {
  const spreadsheet = fetchPubMedWeeklyAndSave_POST();  // 스프레드시트 객체 반환
  if (spreadsheet) {
    SpreadsheetApp.setActiveSpreadsheet(spreadsheet);  // 새로 생성된 스프레드시트를 활성화
    scoreAndFilterPapers(spreadsheet);
    summarizePubMedArticlesWithGPT(spreadsheet);  // 별도 인자 없이 실행
  }
}
...
function fetchPubMedWeeklyAndSave() {
  ...
  const meshTerms = ["drug hypersensitivity","food hypersensitivity","urticaria","angioedema","angioedemas, hereditary","hereditary angioedema","dermatitis,atopic","atopic dermatitis", "eczema","prurigo","vasculitis","anaphylaxis","contact dermatitis","mastocytosis","Stevens-Johnson Syndrome","Toxic Epidermal Necrolysis"];
  const meshQuery = meshTerms.map(term => `"${term}"[MeSH Terms]`).join(" OR ");
  const diseaseTerms = ["urticaria", "angioedema", "atopic dermatitis","eczema","vasculitis","prurigo","anaphylaxis","food allergy","drug allergy","drug hypersensitivity","food hypersensitivity"];
...
/**
 * PubMed에서 최근 1주일간의 논문 검색 및 저장 (POST 방식 사용)
 */
function fetchPubMedWeeklyAndSave_POST() {
  // 1. 검색 쿼리 준비 (기존 코드와 동일)
  const journalQuery = CONFIG.JOURNALS.map(journal => `"${journal}"[Journal]`).join(" OR ");
  const pubTypeQuery = CONFIG.PUB_TYPES.map(type => `"${type}"[Publication Type]`).join(" OR ");
  const meshTerms = ["drug hypersensitivity","food hypersensitivity","urticaria","angioedema","angioedemas, hereditary","hereditary angioedema","dermatitis,atopic","atopic dermatitis","vasculitis", "eczema","prurigo","anaphylaxis","contact dermatitis","mastocytosis","Stevens-Johnson Syndrome","Toxic Epidermal Necrolysis"];
  const meshQuery = meshTerms.map(term => `"${term}"[MeSH Terms]`).join(" OR ");
  const diseaseTerms = ["urticaria", "angioedema", "atopic dermatitis","eczema","prurigo","vasculitis","anaphylaxis","food allergy","drug allergy","drug hypersensitivity","food hypersensitivity"];
  const diseaseQuery = diseaseTerms.map(term => `"${term}"[Title]`).join(" OR ");

  // 2. 날짜 범위 설정 (기존 코드와 동일)
  const today = new Date();
  const oneWeekAgo = new Date();
  oneWeekAgo.setDate(today.getDate() - CONFIG.DAYS_RANGE);
  ...
  // 5. POST 요청 파라미터 객체 생성
  const params = {
    'db': 'pubmed',
    'term': finalQuery,
    'retmode': 'json',
    'retmax': 100,     // <--- 이 값이 정말로 100인지, 또는 다른 양의 정수인지 확인!
    'usehistory': 'y',
    'api_key': getSecret('PUBMED_API_KEY')
  };

  // ***** DEBUGGING START *****
  console.log("DEBUG: params 객체 내용:", JSON.stringify(params)); 
  // ***** DEBUGGING END *****

  // 6. 파라미터 객체를 URL 인코딩된 문자열로 변환 (수정된 코드)
  ...
  // ***** DEBUGGING START *****
  console.log("DEBUG: 생성된 payloadString:", payloadString);
  // ***** DEBUGGING END *****
  ...
  const responseBody = response.getContentText();
   ...
   if (responseCode === 200) {
      console.log("DEBUG: PubMed로부터 받은 원본 ESearch responseBody:\n" + responseBody);
      ...
      if (!esearchResults.esearchresult) {
        console.error('PubMed ESearch 응답 형식이 예상과 다릅니다. 응답:', responseBody);
        return null;
      }
      
      const idList = esearchResults.esearchresult.idlist || [];
      // const webEnv = esearchResults.esearchresult.webenv; // EFetch/ESummary에 필요할 수 있음
      // const queryKey = esearchResults.esearchresult.querykey; // EFetch/ESummary에 필요할 수 있음

      // --- 사용자가 요청한 코드 블록 시작 ---
      // 5. 검색 결과 확인
      if (idList.length === 0) {
        console.log(`최근 ${CONFIG.DAYS_RANGE}일간 "${finalQuery}" 조건으로 검색된 논문이 없습니다.`);
        return null;  // 결과가 없으면 null 반환
      }
      
      // 6. 검색된 논문의 상세 정보 가져오기
      console.log(`총 ${idList.length}개의 논문 ID에 대한 상세 정보를 가져옵니다.`);
      const detailedResults = fetchPubMedData(idList /*, webEnv, queryKey */); // webEnv, queryKey는 필요시 fetchPubMedData에 전달
      ...
      // 7. 결과를 스프레드시트에 저장하고 스프레드시트 객체 반환 (수정된 부분)
      return saveResultsToSheet(detailedResults);
      // --- 사용자가 요청한 코드 블록 끝 ---

    } else {
      console.error(`PubMed ESearch API 호출 실패. 응답 코드: ${responseCode}, Body: ${responseBody}`);
      return null;
    }
  } catch (error) {
    console.error('PubMed 검색 실패 (전체 프로세스 중 예외 발생):', error.toString());
...
function saveResultsToSheet(data) {
  try {
    ...
    const fileName = `cutaneous_wed_${formattedDate}`;
...
function scoreAndFilterPapers(spreadsheet) {
  ...
  const results = rows.map(function(row, i) {
    ...
    try {
      var jsonStr = callGPTJson('Detect keywords in title/abstract:\n\nTitle: "' + title + '"\nAbstract: "' + abstract + '"\n\nCategories:\n1. CUTANEOUS_ALLERGY: urticaria, angioedema, anaphylaxis, food allergy, drug allergy, drug hypersensitivity, food hypersensitivity, mast cell, mastocytosis\n2. DERMATOLOGY: atopic dermatitis, eczema, contact dermatitis, prurigo, vasculitis, Stevens-Johnson, toxic epidermal necrolysis, cutaneous, dermatitis\n\nReturn JSON: {"hasCutAllergyInTitle":true/false,"hasCutAllergyInAbstract":true/false,"hasDermaInTitle":true/false,"hasDermaInAbstract":true/false}');
      ...
      var cutScore = (f.hasCutAllergyInTitle && f.hasCutAllergyInAbstract) ? 5
        : (f.hasCutAllergyInTitle ? 2 : 0) + (f.hasCutAllergyInAbstract ? 2 : 0);
      var dermaScore = (f.hasDermaInTitle && f.hasDermaInAbstract) ? 5
        : (f.hasDermaInTitle ? 2 : 0) + (f.hasDermaInAbstract ? 2 : 0);
      var score = Math.max(cutScore, dermaScore);

      Utilities.sleep(500);
      return {
        details: JSON.stringify(f), score: score,
        included: score >= CONFIG.MIN_RELEVANCE_SCORE,
        reason: "Cut(" + cutScore + ") Derma(" + dermaScore + ") = " + score
      };
    } catch (e) {
...
• 연구 방법: 이 리뷰 연구에서는 IgE의 기도 상피세포에 대한 직접적이고 간접적인 역할에 대해 논의하였으며, 알레르기성 천식 질환에 초점을 맞추었습니다.
• 🎯: IgE는 알레르기성 기도 질환에서 핵심 분자로, <b>T2 inflammation</b> 및 기도 상피 내 리모델링 과정에서 중요한 역할을 합니다. IgE와 기도 상피 간 복잡한 상호작용 네트워크에 대한 더 깊은 이해는 천식 병리생리학에 대한 이해를 향상시킬 것입니다.
• 임상적용 가능성: IgE를 차단하는 <b>omalizumab</b>은 알레르기성 천식 치료에 효과적인 것으로 나타났습니다.
• 제한점: IgE의 <b>기도 상피세포</b>에 대한 역할은 덜 알려져 있습니다.
...
• 주요 대상 질환: Asthma
• 연구 방법: Allergy와 2형(T2) 중재 기도염증의 메커니즘 연구를 통해 천식 치료를 위한 다양한 항체 치료법을 개발하고, 특히 alarmin과 그들의 수용체를 대상으로 한 치료법에 대해 검토하였습니다.
• 🎯: <b>Alarmins</b>과 그들의 수용체를 대상으로 한 치료법은 T2-high 및 T2-low asthma 치료에 효과적일 수 있으며, 이미 <b>tezepelumab</b> 이라는 <b>alarmins</b> 대상 항체가 severe asthma 치료를 위해 승인 받았습니다.
• 임상적용 가능성: <b>Alarmins</b>과 그들의 수용체를 대상으로 한 치료법은 asthma의 정밀 의학 분야에서 새로운 전선을 열 수 있습니다.
...
• 🎯: 아토피 피부염의 병태생리학적 기전에 대한 이해가 크게 향상되었으며, 이를 바탕으로 새로운 표적 치료법이 개발되어 치료 옵션이 혁신적으로 발전함
...
• 👤: Lopes GPR, Ramos YÁL, Pereira NV, et al.
• 주요 대상 질환: Chronic spontaneous urticaria (CSU), Chronic inducible urticaria (CIndU)
• 연구 방법: 만성 두드러기 환자와 건강한 대조군의 피부 조직 샘플을 대상으로, toluidine blue 염색을 통한 mast cell 정량, 면역조직화학법으로 IL-31, IL-31RA, IL-4, IFN-γ 발현 평가, 그리고 flow cytometry 기반 basophil activation test(BAT)를 수행한 관찰 연구
• 🎯: CSU 환자군에서 mast cell 수가 증가하였으며, <b>IL-31</b>과 <b>IL-31RA</b>의 표피 및 진피 발현이 유의하게 상승하였다. 또한 <b>IL-4</b>의 표피 발현도 CSU에서 증가하였고, BAT 양성률이 높아 CSU의 자가면역적 특성을 지지하였다. CIndU에서는 진피 내 <b>IL-31</b> 발현이 증가하였다. 이들 염증 매개체들은 가려움증과 관련된 병태생리에 중요한 역할을 함을 시사한다.
• 임상적용 가능성: CSU에서 관찰된 pruritus biomarkers의 증가와 자가면역적 기전은 향후 진단적 바이오마커 및 면역조절 치료 표적으로 활용될 수 있다.
• 제한점: 소규모 환자군 대상 연구로 일반화에 제한이 있으며, 장기적 임상 결과 및 치료 반응과의 연관성은 추가 연구가 필요하다.
• Tag: #ChronicSpontaneousUrticaria #IL31 #MastCells #BasophilActivationTest #PruritusBiomarkers
